...
const defaultColor = new Map([[true, "#b3cf42"], [false, "#fc6161"]])

const Icone = ({ size, style, color = '#b3cf42', success = true }) => $("svg",
    {
        style: {
            borderColor: color,
            transform: !success && 'rotate(45deg)',
            ...style,
        },
        width: size,
        height: size,
        viewBox: success ? "0 0 20 16" : "0 0 13 13",
        fill: "none",
...
const FeedbackBoxStyles = (success) => ({
    ...
    leftBar: {
        background: defaultColor.get(success),
        width: 10,
        height: 80,
        marginRight: 20,
        flexShrink: 0
...
const FeedbackBox = ({ id, title, message, success, remove }) => {
    ...
    return $("div", { style: Styles.main, ref },
        [
            $("div", { style: Styles.leftBar }),
            $(Icone, { size: 20, style: Styles.icon, success, color: defaultColor.get(success) }),
            $("div", { style: Styles.content },
                [
                    $("h6", { style: Styles.title }, title),
                    $("p", { style: Styles.message }, message),
                ]),
            $("button", { style: Styles.button, onClick: () => remove(id) }, "close"),
...
export const Provider = ({ children }) => {
    ...
    return $(FeedbackContext.Provider, { value: { feedBacks, addFeedBack } },
        [
            ...
            $("div",
                {
                    style: { position: "fixed", bottom: 0, right: 0, padding: 16 },
                    className: "feedback-provider-box"
                },
